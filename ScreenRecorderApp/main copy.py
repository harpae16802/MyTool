...
class ScreenRecorderApp:
    ...
    def on_button_press(self, event):
        logging.debug("鼠標按下: (%d, %d)", event.x, event.y)
    ...
    def on_mouse_drag(self, event):
        ...
        logging.debug("拖動鼠標: (%d, %d) - 寬: %d, 高: %d", event.x, event.y, width, height)

    def on_button_release(self, event):
        logging.debug("鼠標釋放: (%d, %d)", event.x, event.y)
...
def main():
    try:
        ...
        error_message = str(e)
        logging.error("主程序異常: %s", error_message)
